...
export function sanitizeEventForStorage(event, options) {
...
function copyMetadataOnlyFields(event) {
    const output = {
        id: event.id,
...
function sanitizeError(error, maxChars) {
    if (error === undefined) {
        return undefined;
    }
...
function sanitizeDiffHunks(hunks, maxChars) {
    if (hunks === undefined) {
        return undefined;
    }
    return hunks.map((hunk) => {
        const output = { file: sanitizeMetadataString(hunk.file, "path") };
...
function sanitizeMetadataString(value, label) {
