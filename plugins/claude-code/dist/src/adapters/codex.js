...
export function parseCodexHookInput(input, fallbackCwd = processCwd()) {
    const hookEventName = readString(input, "hook_event_name") ?? readString(input, "hookEventName") ?? readString(input, "event") ?? "unknown";
    const sessionId = readString(input, "session_id") ?? readString(input, "sessionId") ?? readString(input, "conversation_id") ?? "codex-session";
...
export function codexHookToAgentEvent(input) {
...
function copyMetadata(source, target, sourceKey, targetKey) {
    if (source[sourceKey] !== undefined) {
        target[targetKey] = toJsonValue(source[sourceKey]);
    }
}
function isCodexApplyPatchFailure(input) {
...
function isCodexUnhandledToolException(input) {
...
function readErrorMessage(source) {
    if (source === undefined) {
        return undefined;
    }
