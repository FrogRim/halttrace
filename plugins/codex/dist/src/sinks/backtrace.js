...
export function renderIncidentMarkdown(incident, options) {
    const lines = [];
    ...
    for (const event of incident.events) {
        lines.push(`| ${escapeTable(event.timestamp)} | \`${event.type}\` | ${escapeTable(event.toolName ?? "")} | ${escapeTable(shorten(event.command ?? "", 80))} | ${event.exitCode ?? ""} |`);
    }
    lines.push("");
...
function renderEventDetails(event, mode, options) {
    const lines = [];
