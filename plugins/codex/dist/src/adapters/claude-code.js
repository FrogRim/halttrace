...
export function parseClaudeHookInput(input, fallbackCwd = processCwd()) {
    const hookEventName = readString(input, "hook_event_name") ?? readString(input, "hookEventName") ?? readString(input, "event") ?? "unknown";
    const sessionId = readString(input, "session_id") ?? readString(input, "sessionId") ?? readString(input, "conversation_id") ?? "claude-session";
...
export function claudeHookToAgentEvent(input) {
...
function isOrdinaryCommandFailure(input) {
    const command = readString(input.toolInput ?? {}, "command") ?? readString(input.toolInput ?? {}, "cmd");
