...
- Use the scenario smoke test for changes that affect Claude hook parsing, trigger policy, dump writing, or wrapper behavior; use the Codex CLI/wrapper tests for Codex stdout and apply_patch failure behavior.
...
- Do not emit host control JSON such as `decision`, `permissionDecision`, `continue:false`, or `retry:true`; Codex observer paths must keep stdout empty.
