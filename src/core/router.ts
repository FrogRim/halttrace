...
import type { AgentEvent, DumpMode, IncidentSnapshot, RouterDiagnostic, RouterResult } from "./types.js";
...
export class AgentEventRouter {
  readonly #store: FileEventStore;
  readonly #deduper: IncidentDeduper;
  readonly #sinks: EventSink[];
  readonly #stateRoot: string;
  readonly #projectHash: string;
  readonly #dumpMode: DumpMode;
  readonly #now: () => Date;
  ...
  async process(event: AgentEvent): Promise<RouterResult> {
  ...
  async dispatchEvent(event: AgentEvent, diagnostics: RouterDiagnostic[]): Promise<void> {
    for (const sink of this.#sinks) {
