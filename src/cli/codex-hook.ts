...
import { codexHookToAgentEvent, parseCodexHookInput } from "../adapters/codex.js";
...
async function main(): Promise<void> {
  try {
    ...
    const hookInput = parseCodexHookInput(object, processCwd());
    const event = codexHookToAgentEvent(hookInput);
    const dumpMode = readDumpMode();
    const stateRoot = env["HALTTRACE_STATE_DIR"] ?? env["PLUGIN_DATA"] ?? env["CLAUDE_PLUGIN_DATA"];
    ...
    if (result.triggered && result.deduped !== true && (await exists(paths.dumpPath))) {
      console.error(`[halttrace] backtrace dump: ${paths.dumpPath}`);
    }
